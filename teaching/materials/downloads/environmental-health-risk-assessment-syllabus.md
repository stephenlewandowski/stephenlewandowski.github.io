# Environmental Health Risk Assessment

## Portfolio Sample Notice

This syllabus is a newly developed teaching-portfolio artifact informed by documented prior teaching experience, course materials, and current professional sources. It is not an official syllabus of a current or former institution, and the activities below are not presented as a record of one historical course offering.

## Course Overview

Environmental health risk assessment turns incomplete evidence into a transparent account of what may cause harm, who or what may be exposed, how large the risk may be, and what remains uncertain. This graduate/professional course develops the full reasoning sequence: problem formulation, conceptual modeling, hazard identification, dose-response evaluation, exposure assessment, risk characterization, and communication for decision support.

The course treats risk assessment as a scientific input to decisions, not the decision itself. Students must distinguish evidence from assumption, variability from uncertainty, regulatory requirements from professional guidance, and risk-assessment conclusions from the value, feasibility, equity, and policy judgments involved in risk management.

## Intended Audience

Graduate students and professionals in environmental health, public health, occupational health, environmental science, environmental engineering, preventive medicine, or related fields. Learners should be comfortable interpreting scientific evidence and performing transparent calculations; no particular software platform is required.

## Course Learning Objectives

By the end of the course, students will be able to:

1. Frame an environmental-health risk problem by specifying the decision context, population, stressor, scope, and analytic boundaries.
2. Construct and defend a conceptual model linking sources, pathways, routes, receptors, outcomes, and potential interventions.
3. Evaluate hazard and dose-response evidence for relevance, quality, limitations, bias, and appropriate use.
4. Design or critique an exposure assessment that identifies populations, routes, magnitude, frequency, duration, data needs, and model assumptions.
5. Integrate hazard, dose-response, and exposure evidence into a risk characterization that distinguishes variability, uncertainty, sensitivity, and data gaps.
6. Distinguish scientific risk assessment from risk-management choices and interpret regulatory requirements, guidance, and recommendations according to their authority and current status.
7. Conduct a transparent, reproducible analysis using documented sources, assumptions, calculations, and versioned work products.
8. Communicate a bounded, defensible risk conclusion and decision-support recommendation to technical and nontechnical audiences.

## Teaching and Learning Approach

Preparation is deliberately manageable: short readings, source checks, concept questions, case background, data previews, or brief written reasoning. Class time is used for application - comparing evidence, drawing conceptual models, checking calculations, testing assumptions, discussing cases, and defending professional judgments.

Four project-generated cases provide a cumulative practice environment:

- **Drinking-water contamination:** A utility must frame a PFAS concern, identify affected populations and exposure pathways, and explain what current requirements do and do not establish.
- **Workplace solvent use:** A maintenance facility must evaluate inhalation and dermal pathways, compare measurement strategies, and prioritize controls without confusing a limit with a complete risk conclusion.
- **Extreme heat and vulnerability:** A regional health partnership must decide where limited preventive resources should go despite uncertain exposure, changing climate conditions, population variability, and incomplete local data.
- **Community vapor-intrusion concern:** A project team must integrate site evidence, building use, model assumptions, community concerns, and alternative management actions in a concise briefing.

Participation can take the form of written reasoning, calculations, source evaluation, small-group analysis, peer critique, discussion, or a defended recommendation. The objective is to make reasoning visible, not to reward speaking frequency.

## Course Materials and Readings

Readings are selected for the problem at hand and checked for authority, scope, and currency. Students will work with current official guidance, peer-reviewed evidence, transparent models, and small public or project-generated datasets. Archived course materials may inform instructor preparation but are not assigned or represented as current authority unless separately reviewed.

Students are expected to maintain an evidence ledger recording source, version or access date, claim supported, limitation, and whether the source is regulation, guidance, recommendation, data, or research evidence.

## Assessment and Grading

| Assessment | Weight | What it demonstrates |
| --- | ---: | --- |
| Preparation and short analyses | 15% | Source review, concept framing, assumptions, and readiness to apply evidence |
| Applied exercises | 20% | Conceptual models, calculations, exposure scenarios, uncertainty analysis, and peer critique |
| Case analysis memorandum | 20% | Integrated reasoning and concise professional communication |
| Cumulative risk-assessment project | 30% | Transparent problem formulation through risk characterization and decision support |
| Final briefing and professional defense | 15% | Audience-centered communication, response to questions, and bounded judgment |
| **Total** | **100%** | |

Assessment emphasizes application rather than a high-stakes recall examination. Criteria include scientific reasoning, source quality, transparency, appropriate methods, treatment of uncertainty, distinction between assessment and management, and communication proportional to the evidence.

## Major Risk Assessment Project

Students develop one bounded environmental-health risk assessment over the term. The project may be presented as a technical report, decision memorandum, risk assessment, or professional briefing, but it must include:

1. A decision-centered problem statement and scope.
2. A conceptual model of sources, pathways, routes, receptors, outcomes, and possible interventions.
3. A documented evidence search and authority map.
4. Hazard and dose-response evaluation appropriate to the question.
5. An exposure assessment with explicit populations, routes, time frame, variables, and assumptions.
6. A risk characterization that integrates evidence and identifies variability, uncertainty, sensitivity, and data gaps.
7. A clear boundary between scientific assessment and risk-management considerations.
8. A reproducibility package containing source citations, methods, calculations, data provenance, and version notes.
9. A bounded recommendation or decision-support product that explains what additional evidence could change the conclusion.
10. A final briefing for a specified professional audience.

Projects use public, de-identified, or synthetic information. They do not authorize human-participant recruitment, private-record access, or field sampling requiring institutional approval.

## Weekly Course Schedule

| Week | Topic | Preparation | In-Class Application | Major Deliverable |
| ---: | --- | --- | --- | --- |
| 1 | Risk, evidence, and decision making | Read an official risk overview; classify sample claims by authority | Diagnose a contested environmental-health decision | Topic shortlist |
| 2 | Problem formulation and conceptual models | Review case background; draft boundaries | Build and critique source-pathway-receptor models | Problem statement |
| 3 | Hazard identification and evidence evaluation | Compare evidence types and study limitations | Weight-of-evidence workshop | Evidence map |
| 4 | Dose-response assessment | Review endpoint and dose-response summaries | Interpret model choices and extrapolation limits | Hazard/dose-response note |
| 5 | Exposure assessment fundamentals | Preview variables and a small dataset | Construct an exposure scenario and calculation audit | Exposure plan |
| 6 | Environmental and occupational pathways | Review drinking-water and workplace cases | Compare routes, time patterns, sampling options, and controls | Applied exercise 1 |
| 7 | Risk characterization | Read two contrasting characterizations | Integrate evidence into a qualified conclusion | Characterization outline |
| 8 | Variability, uncertainty, sensitivity, and assumptions | Complete an assumptions table | Test which assumptions drive the conclusion | Sensitivity memo |
| 9 | Regulation, guidance, and risk management | Classify current sources by authority | Separate scientific findings from management choices | Authority map |
| 10 | Drinking-water contamination case | Review current official PFAS material; identify currentness issues | Develop a bounded utility decision memorandum | Case memorandum |
| 11 | Occupational exposure case | Review hierarchy of controls and exposure guidance | Design a measurement-and-control strategy | Applied exercise 2 |
| 12 | Vulnerability and emerging hazards | Review heat-health evidence and uncertainty | Prioritize action under incomplete local evidence | Project draft |
| 13 | Risk communication and decision support | Draft technical and public summaries | Peer review for accuracy, clarity, and proportionality | Revised assessment |
| 14 | Integrated presentations and synthesis | Prepare briefing and evidence ledger | Deliver, question, defend, and revise recommendations | Final briefing and project |

## Course Policies

### Evidence and attribution

Every consequential claim, dataset, parameter, and regulatory statement must be traceable to a source. Students must distinguish quotation, paraphrase, analysis, and inference; cite the version or access date when content may change; and state when evidence is incomplete or conflicting.

### Responsible use of artificial intelligence

AI tools may be used for brainstorming, editing, coding assistance, or data-workflow assistance when allowed for the assignment. Students remain responsible for verifying sources, documenting methods, checking calculations, protecting sensitive information, interpreting results, and defending conclusions. Material AI contribution must be disclosed. Fabricated sources, undisclosed substitution for required reasoning, and uploading protected or confidential data to unauthorized systems are not permitted.

### Reproducibility and analytical transparency

Submitted analyses must be reproducible from the described inputs and steps. Assumptions, exclusions, transformations, software or tool versions when relevant, and material judgment calls must be recorded. Precision should never imply more certainty than the evidence supports.

### Professional and ethical responsibility

Students must protect privacy, minimize unnecessary sensitive data, represent affected populations respectfully, and avoid implying that a classroom analysis is an official regulatory, clinical, or institutional determination.

## Currentness and Professional Use

Risk frameworks are durable, but regulations, screening values, toxicity information, datasets, and agency recommendations change. Course materials are checked for currency during development, yet students and practitioners must verify controlling requirements and current official sources at the point of use. Proposed rules are not final requirements, recommendations are not regulations, and a screening value is not automatically a complete risk conclusion.

This portfolio syllabus is educational. It does not replace a site-specific assessment, legal advice, institutional review, occupational-safety determination, or agency decision.

## Selected References

- U.S. Environmental Protection Agency. [Risk Assessment](https://www.epa.gov/risk). Current official overview; accessed 2026-08-10.
- U.S. Environmental Protection Agency. [Human Health Risk Assessment](https://www.epa.gov/risk/human-health-risk-assessment). Planning and four-part assessment overview; accessed 2026-08-10.
- U.S. Environmental Protection Agency. [Guidelines for Human Exposure Assessment](https://www.epa.gov/risk/guidelines-human-exposure-assessment). Final agency guidelines, 2019; webpage accessed 2026-08-10.
- U.S. Environmental Protection Agency. [Risk Management](https://www.epa.gov/risk/risk-management). Assessment-management distinction; accessed 2026-08-10.
- U.S. Environmental Protection Agency. [PFAS National Primary Drinking Water Regulation](https://www.epa.gov/sdwa/and-polyfluoroalkyl-substances-pfas). Verify current final requirements and any active proposals at use.
- National Institute for Occupational Safety and Health. [Hierarchy of Controls](https://www.cdc.gov/niosh/hierarchy-of-controls/index.html). Current professional prevention framework; accessed in the portfolio's controlled source set.
- U.S. Global Change Research Program. [Fifth National Climate Assessment, Chapter 15: Human Health](https://nca2023.globalchange.gov/chapter/15/). Climate-health evidence and vulnerability context.

## Portfolio Context

This sample demonstrates a current course design built from documented experience in environmental-health risk assessment, exposure assessment, water, occupational health, climate and health, applied quantitative work, and professional communication. Historical evidence supports the teaching context and many instructional practices; the exact sequence, cases, policy wording, assessment weights, and integrated project are newly generated for this portfolio sample.

